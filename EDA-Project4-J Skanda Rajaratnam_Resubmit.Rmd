...
W11 <- qplot(data = wineRed, x = density, binwidth = .4,color = I('black'), fill = I('#099EE9')) +
        xlab('Density') +
        ylab('Wine Count')
...
I try to get more about dataset, How are they related to each other. Also trying to get a pattern and the behaviour of each features.
Noticed that the ph is a bell curve with the wine count. It is a Standard normal distribution. The Fixed acidity is roughty skewed with wine count, Volatility is also has a Standard normal distribution with the wine count. Citric acid does not seems to be vary with the wine count.

Residual.sugar, chlorides, free.sulfur.dioxide, total.sulfur.dioxide are right skewed distribution with the wine count. They are long tailed in the right side. Medium quality wine are double trible the count than low and high quality. Seems like normaly distributed.No variation in the density.
...
Total acid in the wine is a left skewed distributed. 
...
Analyze the corelation of each acidity with quality. Red wine Quality is increasing with citric acid and fixed acidity but decreasing with volatile acidity.  
...
### Description Two
Analyzing the correlation of each acidity with quality. Red wine Quality is increasing with citric acid and fixed acidity but decreasing with volatile acidity. The fixed.acidity and citric.acid have highly correlated with quality and positive impact at the same time volatile.acidity has negative impact on quality.The more acidity is better the quality of the red wine.
...
Residual Sugar - This is the amount of sugar left in solution after fermentation has finished. Residual sugar and density have a positive strong correlation. R-squard: Pr values 0.99 and 0.91 accordingly. In wine tasting, the term "acidity" refers to the fresh, tart and sour attributes of the wine which are evaluated in relation to how well the acidity balances out the sweetness and bitter components of the wine. I identified Residual sugar, Alcohol, Acidity and Density are strongly correlated togheter influencing the quality of the wine. Alcohol content appeared to be the number one factor for determining a high quality wine. Citric acid and Sulphates had to be in specific amounts in order for alcohol to influence the taste or quality of the wine.

I observed, Wine with high percent of alcohol content and total acidity tends to be rated as high quality wine. Also wine with low % of alcohol with more total acidity tends to be rated as low quality wine. Based on the result, we can see that the acidity in wine and percent of alcohol content are two important components in the quality and taste of Red wine.

Red wines have deep colors, full bodied, stronger, concentrated flavors and smooth mouthfeel tend to stand out from the assortment more than wines with more subtle characteristics. The process of evaluating wines is very subjective, and experts can be biased by their histories and preferences. It is too complex to make the relation between quality and the other variables using the linear model. Therefore in furture, I think, there are better models or methods (Decision tree learning methods) can be use. We can use machine learning to clasify the quality of wine.
